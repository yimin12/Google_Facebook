...
public class TernarySearchDiscrete {
    ...
    static double discreteTernarySearch(int lo, int hi) {
        while (lo != hi) {
            if (hi - lo == 1) return Math.min(f(lo), f(hi)); // two values left
            if (hi - lo == 2) return Math.min(f(lo), Math.min(f(lo + 1), f(hi))); // three values left
            int mid1 = (2 * lo + hi) / 3, mid2 = (lo + 2 * hi) / 3;
            double res1 = f(mid1), res2 = f(mid2);
            if (Math.abs(res1 - res2) < 0.000000001) {
                // more than 3 values left, narrow these two boundary
                lo = mid1;
                hi = mid2;
